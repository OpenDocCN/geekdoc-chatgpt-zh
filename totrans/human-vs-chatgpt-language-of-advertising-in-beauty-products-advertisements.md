...
通过购买或使用本书及其配套文件（“作品”），您同意本许可证授予使用其中包含的内容的许可，包括光盘，但不赋予您对书中/光盘中的任何文本内容的所有权，也不赋予您对其中包含的任何信息或产品的所有权。本许可证不允许未经出版商书面同意将作品上传到互联网或任何网络（无论何种类型）。本作品中包含的任何文本、代码、模拟、图像等内容的复制或传播仅限于并受各自产品的许可条款约束，并且为了复制或网络本作品中包含的任何文本材料（任何媒体）的部分，必须获得出版商或内容所有者的许可。

MERCURY LEARNING AND INFORMATION（“MLI”或“出版商”）以及任何参与制作配套光盘、配套算法、代码或计算机程序（“软件”）以及本作品任何配套网站或软件的人员，不能也不保证通过使用作品内容可能获得的性能或结果。作者、开发者和出版商已尽最大努力确保本包中包含的文本材料和/或程序的正确性和功能；然而，我们对此类内容或程序的性能不做任何明示或默示的保证。本作品“按原样”出售，不提供任何保证（除非因制造书籍所使用的材料有缺陷或因工艺缺陷）。

本作品的作者、开发者、出版商以及任何参与本作品创作、制作和制造的人员，对于因使用（或无法使用）本出版物中包含的算法、源代码、计算机程序或文本材料而产生的任何类型的损害，均不承担任何责任。这包括但不限于收入或利润的损失，或其他由使用本作品而产生的偶然、物理或后果性损害。

在任何类型的索赔事件中，唯一的补救措施明确限于替换书籍和/或光盘，并且仅由出版商决定。关于“默示保证”和某些“排除条款”，各州可能有所不同，可能不适用于本产品的购买者。

本标题的配套文件可通过向出版商写信并提供购买证明来获取，联系邮箱为 info@merclearning.com。

![image](img/human-vs-chatgpt-language-of-advertising-in-beauty-products-advertisements-titleimg.jpg)
...
出版社承认并尊重公司、制造商和开发者使用的所有标志，作为区分其产品的手段。本书中提到的所有品牌名称和产品名称均为其各自公司的商标或服务标志。任何遗漏或误用（任何形式的）服务标志或商标等，都不是侵犯他人财产的企图。

美国国会图书馆控制号：2024935754

242526321 本书在美国使用无酸纸印刷。

我们的书可供机构、公司等采用、许可或批量购买。如需更多信息，请联系客户服务部，电话：800-232-0223（免长途话费）。

我们的所有标题都以数字格式在[academiccourseware.com](http://academiccourseware.com)和其他数字供应商处提供。通过联系 info@merclearning.com 并提供购买证明，可以获得本标题的配套文件（图表和代码列表）。MERCURY LEARNING AND INFORMATION 对购买者的唯一义务是，根据有缺陷的材料或工艺不良更换文件，但不基于产品的操作或功能。
...
使用 NumPy 和 Matplotlib 绘制的二次散点图
