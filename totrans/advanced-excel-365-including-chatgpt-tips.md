...
通过购买或使用本书及其配套文件（“作品”），您同意本许可证授予您使用其中包含的内容的许可，包括光盘，但并不赋予您对书中/光盘中的任何文本内容的所有权，也不赋予您对其中包含的任何信息或产品的所有权。本许可证不允许未经出版商书面同意将作品上传到互联网或任何网络（无论何种类型）。任何文本、代码、模拟、图像等内容的多重复制或传播仅限于并受各自产品的许可条款约束，并且为了复制或网络作品中的任何部分文本材料（任何媒体），必须从出版商或内容所有者等处获得许可。

MERCURY LEARNING AND INFORMATION（“MLI”或“出版商”）以及参与配套光盘、伴随算法、代码或计算机程序（“软件”）的创建、编写或制作，以及任何伴随作品网站或软件的人员，不能也不保证通过使用作品内容可能获得的性能或结果。作者、开发者和出版商已尽最大努力确保本包中包含的文本材料和/或程序的准确性和功能性；然而，我们不对这些内容的性能或程序做出任何类型的保证，无论是明示的还是默示的。作品“按原样”销售，不提供任何保证（除非因制造书籍所使用的材料有缺陷或因工艺缺陷）。

任何伴随内容的作者、开发者和出版商，以及参与本作品的构思、制作和制造的人员，不对因使用（或无法使用）本出版物中包含的算法、源代码、计算机程序或文本材料而产生的任何类型的损害负责。这包括但不限于收入或利润的损失，或其他由使用本作品而产生的偶然性、物理性或后果性损害。

在任何索赔的情况下，唯一的补救措施是明确限于替换书籍和/或文件，并且仅由出版商决定。关于“默示保证”和某些“排除条款”的规定因州而异，可能不适用于本产品的购买者。

配套文件（彩色插图）可通过向出版商（附购买证明）发送电子邮件至 info@merclearning.com 进行下载。
...
![image](img/advanced-excel-365-including-chatgpt-tips-ML-LOGO.jpg)

MERCURY LEARNING AND INFORMATION

波士顿，马萨诸塞州

版权所有 © 2024 MERCURY LEARNING AND INFORMATION。

德古特公司的一个分支。保留所有权利。经许可重印和修订。

原标题和版权：Mastering Advanced Excel: With ChatGPT Integration.

版权 © 2023 由 BPB Publications 出版。保留所有权利。ISBN：978-93-5551-865-1。

本出版物、其部分内容或任何随附软件不得以任何方式复制、存储在任何类型的检索系统中或通过任何手段、媒体、电子显示或机械显示传输，包括但不限于影印、录音、互联网发布或扫描，除非事先获得出版商的书面许可。
...
出版社承认并尊重公司、制造商和开发者使用的所有标志，作为区分其产品的手段。本书中提到的所有品牌名称和产品名称均为其各自公司的商标或服务标志。任何遗漏或滥用（任何形式）服务标志或商标等，均非侵犯他人财产的企图。

美国国会图书馆控制号：2024939128

242526321 这本书是在美国使用无酸纸印刷的。

我们的书可供机构、公司等采用、许可或批量购买。如需更多信息，请联系客户服务部 800-232-0223（免长途话费）。

所有我们的标题都可在[academiccourseware.com](http://academiccourseware.com)和其他数字供应商处以数字格式获取。配套文件（彩色图表）可通过致信出版社 info@merclearning.com 下载（需提供购买证明）。MERCURY LEARNING AND INFORMATION 对购买者的唯一义务是基于材料缺陷或工艺不良更换文件，但不基于产品的操作或功能。

献给我亲爱的父亲，Ramesh Dhingra，
